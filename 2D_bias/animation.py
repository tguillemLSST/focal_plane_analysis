...
input_path = '/sps/lsst/users/tguillem/web/batch/dark/ccd_assembly_fix/v4_2D/13161/after_master_bias/'
    ...
    for k in range(len(ccds)):
        with imageio.get_writer(input_path + 'gif/' + rafts[j] + '_' + ccds[k] + '.gif', mode='I', fps=1/2) as writer:
            for i in [0,160,165,170,175]:
                ...
                if(i==0):
                    filename = '/sps/lsst/users/tguillem/web/batch/overscan/dark/v4_2D/13161/after_master_bias/000140/' + rafts[j] + '/' + ccds[k] + '/CCD_RawImage_2D_corr.png'
                else:
